...
function present(profile, serviceCatalog = []) {
...
export class DiaristaSearchService {
  async list({ page = 1, limit = 20, nome, bairro, cidade, estado, avaliacao_min, preco_min, preco_max, id_servico, ordenar = 'avaliacao' } = {}) {
    const serviceFilter = {};
    ...
    const profiles = rawProfiles.map((profile) => present(profile, serviceCatalog));
    const sorters = {
