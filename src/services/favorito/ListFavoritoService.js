...
export class ListFavoritoService {
  ...
  async execute({ page = 1, limit = 20 } = {}, auth) {
    ...
    const [data, total] = await Promise.all([
      this.repository.findAll({ where, include: { diarista: { include: { usuario: { select: { nome: true, foto_perfil: true } }, diarista_servico: { include: { servico: true } } } } }, skip: (page - 1) * limit, take: limit }),
